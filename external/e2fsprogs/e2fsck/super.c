...
#define API_LEVEL_TOS 33
	if (sb->s_snapshot_r_blocks_count != API_LEVEL_TOS) {
		log_out(ctx, _("s_snapshot_r_blocks_count is %llu."
			" It might be upgraded from S OS. Reinitialize it to 33\n"),
			(unsigned long long)sb->s_snapshot_r_blocks_count);
		sb->s_snapshot_r_blocks_count = API_LEVEL_TOS;
